...
class HatchetPluginError(RuntimeError):
...
class HatchetTransport(Protocol):
    """Narrow external-runtime operations used by the plugin policy layer."""
...
class HatchetSDKTransport:
    ...
    async def run(
        ...
        *,
        ...
        payload = _copy_mapping(job_input, "job input")
    ...
    async def status(self, job: HatchetRun) -> HatchetRunStatus:
        ...
        try:
            ...
            failure = HatchetPluginError("Hatchet returned an unsupported run status")
        raise failure
    ...
    async def result(self, job: HatchetRun) -> Mapping[str, Any]:
        ...
        return _copy_mapping(result, "Hatchet result")
    ...
    def _runs(self) -> Any:
        ...
        if self._client is None:
            raise HatchetPluginError("Hatchet client is closed")
    ...
    async def _call(self, operation: str, awaitable: Any) -> Any:
        ...
        failure: HatchetPluginError | None = None
        try:
            return await awaitable
        except asyncio.CancelledError:
            raise
        except Exception as error:
            failure = HatchetPluginError(
...
async def open_hatchet_transport(scopes: Sequence[str]) -> HatchetTransport:
    ...
    if not isinstance(material, str) or not material:
        raise HatchetPluginError("Hatchet credential must contain a token string")
...
async def open_hatchet_recovery_transport() -> HatchetTransport:
    ...
    invocation credential context does not exist during plugin startup. The
...
def _create_sdk_client(
    token: str,
) -> tuple[Any | None, HatchetPluginError | None]:
    ...
    try:
        ...
        return Hatchet(config=ClientConfig(token=token)), None
    except Exception as error:
        return None, HatchetPluginError(
            f"Hatchet client initialization failed with {type(error).__name__}"
        )


def _copy_mapping(value: Any, label: str) -> dict[str, Any]:
    """Accept JSON-shaped mappings without retaining an SDK-owned container."""
