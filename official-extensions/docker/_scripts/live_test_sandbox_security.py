...
Install ``harnest-extension-docker`` and run
``python official-extensions/docker/_scripts/live_test_sandbox_security.py
...
from harnest.sandbox import Sandbox, SandboxNetworkPolicy, SandboxRequest
...
def _backend(args: argparse.Namespace) -> Any:
    ...
    # These white-box security probes inspect the extension-owned implementation
    # after constructing it exclusively through the published declaration API.
    return provider._backend
