...
``python official-extensions/docker/_scripts/live_test_sandbox_container.py
--image python:3.12-slim --base-url unix:///path/to/docker.sock``. The image must
already exist. A child owns all providers so process cleanup is observable.
"""
...
def _container_details(backend: Any) -> dict[str, Any]:
    ...
    backend = getattr(backend, "_backend", backend)
...
class RecordingProvider:
    ...
    def __init__(self, backend: Any) -> None:
        ...
        self.backend = getattr(backend, "_backend", backend)
