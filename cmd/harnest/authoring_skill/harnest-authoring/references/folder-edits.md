...
| Add same-process SDK behavior, lifecycle, or declared managed content | Root `extensions/<name>/extension.yaml`, `extension.py`, and package content | This is a Harnest Extension. Export `extension`, list projected directories under `contributes`, declare dependencies/capabilities, and consume its public API below `harnest.extensions.<name>`. |
...
| Add authentication, invocation/model policy, persistence, guardrails, or event transforms | Root `lifecycle/**/*.py` | Decorate executable listeners with `@lifecycle.*`; use one `@lifecycle.output_policy` factory when public subagent narration, tool activity, provider reasoning, or model metadata should differ from the defaults. |
...
| Add sandboxed code execution | `sandbox/<name>.py`, each allowed agent's declaration, and its business tool | Export a matching `Sandbox` variable; assign `Agent(sandboxes=["<name>"])`; call `context.sandboxes["<name>"].execute(code)` or async `aexecute` from an authored tool. No model tool is added automatically. Root names are available to same-project subagents, but access is never inherited. Install a provider extension and use its public API; for Docker run `harnest extensions install docker --project <agent-root>`, import `docker` from `harnest.extensions.docker`, and pass a `SandboxNetworkPolicy` to `docker.sandbox(...)`. Core `Sandbox.provider(...)` remains available for authored providers. |
