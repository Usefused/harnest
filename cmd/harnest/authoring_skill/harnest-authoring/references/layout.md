...
| `lifecycle/credentials.py` | Optional root `@lifecycle.credential_provider` factory returning one private `CredentialProvider`; never publish it with `@context.provider`. |
...
| `lifecycle/**/*.py` | Other arbitrary public root modules; explicit `@lifecycle.*` listeners and `@context.provider` providers are discovered. Multiple listeners may share an invocation phase. |
